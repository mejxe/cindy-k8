...
export default function ClientPlayer(player: Player, me: Player, handlerFunc: () => void = () => { }) {
  const avatarCircle = () => {
    if (player.id === me.id) {
      return "ME"
    }
    else if (player.firstName.at(0) !== undefined && player.lastName.at(0) !== undefined) {
      return player.firstName.at(0) && player.lastName.at(0)
    }
    else {
      return "AC"
    }
